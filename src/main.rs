...
fn main() {
    ...
    tray.add_menu_item("启动", || {
        std::thread::spawn(move || {
            ...
            let check_db_path = home_dir().expect("获取用户目录失败").join("Library/Messages/chat.db-wal");
            let mut last_metadata_modified = fs::metadata(&check_db_path).expect("获取元数据失败").modified().unwrap();
            loop{
                let now_metadata = fs::metadata(&check_db_path).expect("获取元数据失败").modified().unwrap();
